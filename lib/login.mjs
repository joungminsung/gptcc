...
async function loginBrowser() {
  ...
  const codePromise = new Promise((resolve, reject) => {
    ...
    server.listen(REDIRECT_PORT, REDIRECT_HOST, () => {
      // Match numman-ali/opencode-openai-codex-auth's createAuthorizationFlow()
      // exactly — it's a Node.js OAuth implementation that is confirmed
      // to log in successfully from a plain ChatGPT Plus/Pro account.
      //
      // Two reversals from our earlier guesses:
      //   1. Use URLSearchParams (space → "+"). The `%20` form from v2.2.3
      //      is standards-correct but opencode ships `+` and works, so it
      //      isn't the blocker we thought it was.
      //   2. Match opencode's parameter order exactly. OAuth servers
      //      shouldn't care about order in theory, but we ran out of
      //      other differences to rule out.

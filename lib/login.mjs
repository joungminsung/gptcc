...
// Scope must be identity-only for the public Codex CLI client_id from
// a regular ChatGPT Plus/Pro account. The Rust Codex CLI also requests
// `api.connectors.read api.connectors.invoke`, but those scopes are
// only granted to Codex-service accounts — requesting them on a plain
// ChatGPT account is rejected by the authorize server with
// `missing_required_parameter` (OpenAI's error mapping surfaces a
// rejected scope under that name). The well-tested Node.js reference
// implementation (numman-ali/opencode-openai-codex-auth) also uses the
// short form and logs in successfully.
...
async function loginBrowser() {
  ...
  const codePromise = new Promise((resolve, reject) => {
    ...
    server.listen(REDIRECT_PORT, REDIRECT_HOST, () => {
      // Parameter set must match openai/codex's build_authorize_url()
      // exactly. Dropping or renaming any of these results in
      // `missing_required_parameter` from the authorize server.
      //
      // IMPORTANT: do NOT use URLSearchParams / URL.searchParams for the
      // query string. URLSearchParams encodes space as `+`, but OpenAI's
      // authorize server reads `+` in the query as a literal plus. That
      // makes our scope arrive as `openid+profile+email+...` — treated
      // as a single unknown scope and reported back as a missing
      // parameter. Use encodeURIComponent (which yields %20 for space),
      // matching Rust's urlencoding crate used by the official Codex CLI.
      const params = [
        ["response_type", "code"],
        ["client_id", CLIENT_ID],

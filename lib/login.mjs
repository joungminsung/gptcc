...
async function loginBrowser() {
  ...
  const codePromise = new Promise((resolve, reject) => {
    ...
    server.listen(REDIRECT_PORT, REDIRECT_HOST, () => {
      ...
      // `missing_required_parameter` from the authorize server. In
      // particular `codex_cli_simplified_flow=true` tells OpenAI this is
      // a CLI-style consent flow (shorter screen, no web app consent
      // surface).

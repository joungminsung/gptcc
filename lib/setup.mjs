...
export async function setup(options = {}) {
  ...
  if (!proxyRunning) {
    // Windows: Node's spawn({detached: true}) on its own doesn't fully
    // detach a child on Windows — the child remains tied to the parent
    // console and dies when the parent exits. The documented workaround
    // is to go through cmd.exe's `start /b` which owns the detach
    // semantics. POSIX can use the straight spawn() path.
    const spawnEnv = {
      ...process.env,
      HOME,
      GPT_PROXY_PORT: port,
    ...
    if (IS_WINDOWS) {
      // `start "" /B <node> <proxy>`
      //   "" — empty window title (required by start when first arg is quoted)
      //   /B — no new window (background)
      // Use shell: true so cmd.exe parses `start` correctly.
      const cmd = `start "" /B "${nodePath}" "${INSTALLED_PROXY}"`;

...
function copyProxyToInstallDir() {
  ...
  //
  // v2.4: proxy.mjs now also imports config.mjs (Fastmode/Auto wiring) and
  // gpt-native.mjs (GPT-specific request sanitization). Forgetting either
  // breaks a globally-installed gptcc with an opaque module-not-found at
  // startup, hence the explicit list.
  const PROXY_RUNTIME_FILES = ["proxy.mjs", "routing.mjs", "config.mjs", "gpt-native.mjs"];
  ...
  // v2.4: proxy.mjs reads plugin/skills/gptcc-auto-delegate/SKILL.md at
  // runtime (for the /model `claude-auto-opus` alias) via a path relative
  // to its own directory: `<proxyDir>/../plugin/skills/...`. When proxy.mjs
  // lives in INSTALL_DIR (~/.local/share/gptcc/), that resolves to
  // ~/.local/share/plugin/skills/... — so the skill must be copied there
  // alongside the runtime modules. The plugin is ALSO installed under
  // ~/.claude/plugins/gptcc/ (for Claude Code itself), but the proxy has
  // no knowledge of that path, so we duplicate the skill file here.
  const skillSrcDir = join(PROJECT_ROOT, "plugin", "skills", "gptcc-auto-delegate");
  const skillDstDir = join(dirname(INSTALL_DIR), "plugin", "skills", "gptcc-auto-delegate");
  const skillSrc = join(skillSrcDir, "SKILL.md");
  const skillDst = join(skillDstDir, "SKILL.md");
  if (!existsSync(skillSrc)) {
    throw new Error(
      `Packaging bug: plugin/skills/gptcc-auto-delegate/SKILL.md missing from gptcc install. ` +
      `Reinstall with: npm install -g gptcc@latest`
    );
  }
  ...
  if (!existsSync(skillDst)) {
    throw new Error(`Failed to copy SKILL.md to ${skillDst}`);
  }
...
// Detects whether an existing settings.json was written by gptcc v2.3 (or
// older), so setup() can print a migration notice about what changed. v2.3
// registered `gpt-5.4-fast` as a standalone /model entry; v2.4 removes it
// in favour of the Fastmode toggle. Presence of the literal string
// "gpt-5.4-fast" in the pre-modification JSON is a reliable signal.
...
export async function setup(options = {}) {
  ...
  // v2.4: bootstrap ~/.gptcc/config.json with defaults if it doesn't exist.
  // Dynamic import to match the existing helper-import style (login is imported
  // at the top, but config is a cold dependency only needed by setup/uninstall,
  // so we avoid the top-level cost).
  {
    const { writeConfig, defaultConfigPath, DEFAULTS } = await import("./config.mjs");
    const cfgPath = defaultConfigPath();
    if (!existsSync(cfgPath)) {
      writeConfig(cfgPath, { ...DEFAULTS });
      console.log(`  Created ${cfgPath} with default settings.`);
    }
  }
  ...
  let settings = {};
  let settingsJsonContents = ""; // raw pre-mod text, for v2.3→v2.4 detection
  if (existsSync(SETTINGS_PATH)) {
    try {
      settingsJsonContents = readFileSync(SETTINGS_PATH, "utf-8");
      settings = JSON.parse(settingsJsonContents);
    } catch {}
  }
  ...
  // Mode selection. Three modes, all using only documented extension
  // points (no binary modification, ToS-safe):
  //
  //   single-slot (default) — Claude slots untouched, GPT added as a
  //                           5th entry in the /model picker. User picks
  //                           GPT manually when they want it. Claude's
  //                           Default / Opus / Sonnet / Haiku all work
  //                           unchanged. This is what most users want.
  //
  //   multi-slot            — replace ALL three slots + CUSTOM with GPT.
  //                           Claude models disappear from the picker.
  //                           For users who want a pure-GPT experience.
  //
  //   hybrid                — remap the Sonnet + Haiku slots to GPT and
  //                           leave Opus as Claude. Default resolves to
  //                           GPT on Pro accounts. Opt-in only via
  //                           --hybrid flag because it changes what
  //                           Default points at.
  const mode =
    options.multiSlot || process.env.GPTCC_MULTI_SLOT === "1"
      ? "multi-slot"
      : options.hybrid || process.env.GPTCC_HYBRID === "1"
      ? "hybrid"
      : "single-slot";

  // v2.4: single-slot default is now `claude-auto-opus` (the "Auto" entry —
  // proxy rewrites to real Opus and prepends the gptcc-auto-delegate skill
  // as a system block so Opus delegates to GPT-5.4 on its own). Previous
  // default was `gpt-5.4-fast`, but the standalone Fast entry has been
  // replaced by the runtime Fastmode toggle (`gptcc setting fastmode on`).
  const defaultModel = options.model || process.env.GPTCC_DEFAULT_MODEL || "claude-auto-opus";

  // Clear every slot-level env var from all prior modes before re-writing,
  // so switching modes doesn't leave stale entries.
  for (const k of [
  ...
  if (mode === "multi-slot") {
    ...
    // Remap Sonnet + Haiku slots to GPT variants. Opus stays Claude so
    // the heaviest Claude model is always one manual pick away. No
    // Bedrock flag — we stay in native Anthropic API mode and the proxy
    // routes based on model-ID prefix (claude-* passthrough, gpt-*
    // translate to Codex backend).
    settings.env.ANTHROPIC_DEFAULT_SONNET_MODEL = "gpt-5.4-fast";
    settings.env.ANTHROPIC_DEFAULT_SONNET_MODEL_NAME = "GPT-5.4 Fast";
    settings.env.ANTHROPIC_DEFAULT_SONNET_MODEL_DESCRIPTION = "OpenAI · priority tier · 1.5× speed";
    settings.env.ANTHROPIC_DEFAULT_SONNET_MODEL_SUPPORTED_CAPABILITIES =
      "effort,thinking,adaptive_thinking,interleaved_thinking";

    settings.env.ANTHROPIC_DEFAULT_HAIKU_MODEL = "gpt-5.4-mini";
    settings.env.ANTHROPIC_DEFAULT_HAIKU_MODEL_NAME = "GPT-5.4 Mini";
    settings.env.ANTHROPIC_DEFAULT_HAIKU_MODEL_DESCRIPTION = "OpenAI · lightweight · fast";

    // Heaviest GPT in the CUSTOM slot for one-click access.
    settings.env.ANTHROPIC_CUSTOM_MODEL_OPTION = "gpt-5.4";
    settings.env.ANTHROPIC_CUSTOM_MODEL_OPTION_NAME = "GPT-5.4";
    settings.env.ANTHROPIC_CUSTOM_MODEL_OPTION_DESCRIPTION = "OpenAI flagship · 1M ctx · heaviest";
    settings.env.ANTHROPIC_CUSTOM_MODEL_OPTION_SUPPORTED_CAPABILITIES =
      "effort,xhigh_effort,max_effort,thinking,adaptive_thinking,interleaved_thinking";
  } else {
    // single-slot — conservative, 5th-option-only install.
    //
    // v2.4 entries (see README §/model picker):
    //   • claude-auto-opus  →  "Auto"      — Opus + delegation skill (default)
    //   • gpt-5.4-auto      →  "GPT-5.4"   — proxy resolves to -fast when
    //                                         Fastmode is on
    //
    // ANTHROPIC_CUSTOM_MODEL_OPTION is a single-slot env var — it holds
    // exactly one picker entry. We register `defaultModel` there and that
    // is the one pickable entry gptcc adds. To swap the slot, users re-run
    // `gptcc setup --model <id>`; to get multiple entries at once, they
    // opt in via `gptcc setup --multi-slot` or `--hybrid`.
  ...
  console.log(`  Settings updated (mode: ${mode})`);
  ...
  // v2.3 → v2.4 migration notice. Only prints when the settings.json we
  // just overwrote was a v2.3 install (detected via presence of the
  // standalone `gpt-5.4-fast` picker entry in the pre-modification text).
  ...
  if (mode === "single-slot") {
    const primaryName = prettyModelName(defaultModel);
    const primaryDesc =
      defaultModel === "claude-auto-opus"
        ? "Claude Opus + delegate to GPT"
        : defaultModel === "gpt-5.4-auto"
        ? "OpenAI flagship (Fastmode-aware)"
        : modelDescription(defaultModel);
    console.log("  /model picker now shows (Claude models unchanged):");
    console.log("    Default / Opus / Sonnet / Haiku  ← Claude, as before");
    console.log(`    ${primaryName.padEnd(32)} ← ${primaryDesc}`);
    console.log("");
    console.log("  Tip: To swap the single picker slot, re-run");
    console.log("    gptcc setup --model <id>   (ids: claude-auto-opus, gpt-5.4-auto)");
    console.log("  For both entries simultaneously, use:");
    console.log("    gptcc setup --multi-slot");
    console.log("");
    console.log("  Toggle speed: `gptcc setting fastmode on` (routes GPT-5.4 to -fast).");
    console.log("  Advanced: `gptcc setup --hybrid` to make Default route to GPT,");
    console.log("  or `gptcc setup --multi-slot` for an all-GPT picker.");
  } else if (mode === "hybrid") {
    console.log("  /model picker now shows:");
    console.log("    Default (auto)  → GPT-5.4 Fast   ← most accounts (Pro tier)");
    console.log("    Opus            → Claude Opus    ← unchanged");
    console.log("    Sonnet          → GPT-5.4 Fast   ← remapped");
    console.log("    Haiku           → GPT-5.4 Mini   ← remapped");
    console.log("    GPT-5.4         → GPT-5.4 (1M ctx, heaviest)");
    console.log("");
    console.log("  On Max / Team Premium (Default=Opus), Default stays Claude Opus.");
  } else {
    console.log("  Mode: multi-slot — GPT-5.4 / GPT-5.4 Fast / GPT-5.4 Mini / GPT-5.3 Codex");
    console.log("  all appear in the /model picker. Claude models are hidden.");
  }
  console.log("");
}
...
export async function uninstall() {
  ...
  // v2.4: the proxy reads the gptcc-auto-delegate skill from a sibling
  // `plugin/` dir next to INSTALL_DIR. Remove just that skill's dir; if
  // the parent `plugin/` dir is left empty afterwards, remove it too.
  const parallelSkillDir = join(dirname(INSTALL_DIR), "plugin", "skills", "gptcc-auto-delegate");
  if (existsSync(parallelSkillDir)) {
    try {
      rmSync(parallelSkillDir, { recursive: true, force: true });
    } catch {}
  }
  const parallelPluginSkills = join(dirname(INSTALL_DIR), "plugin", "skills");
  const parallelPluginDir = join(dirname(INSTALL_DIR), "plugin");
  for (const dir of [parallelPluginSkills, parallelPluginDir]) {
    try {
      // rmdir-on-empty semantics: rmSync without force will throw ENOTEMPTY
      // when other files live here, so unrelated data in ~/.local/share/plugin/
      // (if any) is preserved.
...
function prettyModelName(id) {
  const map = {
    // v2.4 picker entries
    "claude-auto-opus": "Auto",
    "gpt-5.4-auto": "GPT-5.4",
    // Direct model IDs (still valid targets, just not in the standard picker)
    "gpt-5.4": "GPT-5.4",
...
function modelDescription(id) {
  const map = {
    // v2.4 picker entries
    "claude-auto-opus": "Claude Opus + auto-delegate to GPT-5.4",
    "gpt-5.4-auto": "OpenAI flagship · 1M ctx · Fastmode-aware",
    // Direct model IDs

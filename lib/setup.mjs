...
export async function setup(options = {}) {
  ...
  try {
    ...
    if (data.proxy === "gptcc") {
      if (pkgVersion && data.version && data.version !== pkgVersion) {
        ...
        // Wait for the port to free up. Old binaries without /shutdown
        // will time out here and we just move on — spawn will fail on
        // EADDRINUSE and the user gets a clear error.
        for (let i = 0; i < 20; i++) {
  ...
  try {
    ...
    // shell:true fails to resolve .cmd / .bat shims, and its error gets
    // silently swallowed by our old catch. shell:true makes cmd.exe do
    // the extension-search, which handles both real .exe and .cmd shims.
    ...
    if (result.status === 0) {
      ...
      const detail = stderr || stdout || `exit ${result.status}${result.error ? ` (${result.error.code || result.error.message})` : ""}`;

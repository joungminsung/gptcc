...
export async function setup(options = {}) {
  ...
  try {
    // On Windows, the Claude Code "binary" is often a .cmd shim (`claude.cmd`)
    // even though the install path ends in `.exe`. spawnSync without
    // shell:true fails to resolve .cmd / .bat shims. shell:true makes
    // cmd.exe do the extension-search, which handles both real .exe and
    // .cmd shims.
    ...
    if (result.status === 0) {
      ...
      // Claude Code CLI versions without `plugin add` support print
      // "unknown command 'add'". Plugin registration is only used for
      // the SessionStart hook that auto-starts the proxy; since setup
      // already started the proxy, this is a cosmetic miss, not a
      // functional problem. Treat it as informational.
      const unsupported = /unknown command|not a git command/i.test(detail);
      if (unsupported) {
        console.log("  Plugin registration skipped — this Claude Code build doesn't expose");
        console.log("  `plugin add`; the proxy is already running and will keep working.");
      } else {
        console.log("  Plugin registration skipped");
        if (detail) {
          console.log(`    reason: ${detail.split("\n")[0].slice(0, 200)}`);
        }

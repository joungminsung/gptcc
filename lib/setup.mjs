...
function copyProxyToInstallDir() {
  ...
  // proxy.mjs imports `./routing.mjs` (pure helpers extracted for
  // testability). Both files must live side-by-side in the install
  // dir or the proxy fails with ERR_MODULE_NOT_FOUND on startup.
  copyFileSync(
    join(PROJECT_ROOT, "lib", "routing.mjs"),
    join(INSTALL_DIR, "routing.mjs")
  );

...
export async function setup(options = {}) {
  ...
  //
  //   hybrid (default)  — remap the Sonnet + Haiku slots to GPT and leave
  //                       Opus as Claude. Because Claude Code's "Default"
  //                       setting is a static pointer to the Sonnet slot
  //                       on Pro / Anthropic-API accounts (to Opus on Max /
  //                       Team Premium), remapping Sonnet makes Default
  //                       automatically resolve to GPT without user action,
  //                       while the full Claude Opus stays one click away
  //                       via manual pick. Best of both worlds.
  //
  //   multi-slot        — replace ALL three slots + CUSTOM with GPT.
  //                       Claude models disappear from the picker. For
  //                       users who want a pure-GPT experience.
  //
  //   single-slot       — leave all Claude slots intact and add GPT as a
  //                       5th CUSTOM option. Most conservative; GPT is
  //                       manual-pick only. What v2.0 shipped as default.
  const mode =
    options.multiSlot || process.env.GPTCC_MULTI_SLOT === "1"
      ? "multi-slot"
      : options.singleSlot || process.env.GPTCC_SINGLE_SLOT === "1"
      ? "single-slot"
      : "hybrid";
  ...
  if (mode === "hybrid") {
    ...
    console.log("");
    console.log("  If your account's Default points to Opus (Max / Team Premium):");
    console.log("    run `gptcc setup --default-slot=opus` to remap Opus instead.");
  } else if (mode === "multi-slot") {
    ...
    console.log(`  Mode: single-slot — picker shows ${defaultModel} as a 5th manual option.`);
    console.log('  Default still picks Claude. Run `gptcc setup` (no flags) for hybrid mode.');
  }
  console.log("");
}

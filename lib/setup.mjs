...
export async function setup(options = {}) {
  ...
  if (mode === "multi-slot") {
    ...
    //                                         Fastmode is on
    //
    // ANTHROPIC_CUSTOM_MODEL_OPTION only holds one entry, so we register
    // `defaultModel` there and list any companion entries via
    // `availableModels` so they still show in the picker.
    ...
    // Companion entries in the picker. Only the ones that aren't already the
    // default get added — no duplicates.
    const companions = ["claude-auto-opus", "gpt-5.4-auto"].filter((m) => m !== defaultModel);
    const existing = Array.isArray(settings.availableModels) ? settings.availableModels : [];
    // Drop stale v2.3 entries before re-adding the v2.4 set.
    const filtered = existing.filter(
      (m) => !["gpt-5.4", "gpt-5.4-fast", "claude-auto-opus", "gpt-5.4-auto"].includes(m)
    );
    settings.availableModels = [...filtered, ...companions];
  }
  ...
  if (mode === "single-slot") {
    ...
    console.log("    Auto                             ← Claude Opus + delegate to GPT");
    console.log("    GPT-5.4                          ← OpenAI flagship (Fastmode-aware)");
    console.log("");

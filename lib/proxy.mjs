...
function checkProxyAuth(req) {
  ...
  const hdr = req.headers["x-gptcc-auth"] || req.headers["authorization"] || "";
  const val = hdr.replace(/^Bearer\s+/i, "");
  return val === PROXY_AUTH_TOKEN;
}

// ---------------------------------------------------------------------------
// Bedrock-format route parsing
// `/model/<id>/invoke` and `/model/<id>/invoke-with-response-stream`
// are what Claude Code sends in CLAUDE_CODE_USE_BEDROCK=1 mode. We accept
// both URL-encoded model IDs and treat the body as Anthropic Messages
// format (Claude Code always sends Anthropic format even in Bedrock mode).
// ---------------------------------------------------------------------------

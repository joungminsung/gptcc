...
import {
  parseBedrockInvoke,
  isOpenAIModel as _isOpenAIModelBase,
  checkProxyAuth as _checkProxyAuthBase,
  resolveGptModel,
  isAutoAlias,
  AUTO_REAL_MODEL,
} from "./routing.mjs";
...
// ============================================================================
// Section 0: buildUpstreamRequest — v2.4 config cache + Auto/Fast mode wiring
// ============================================================================
//
// Pure-ish transform from raw incoming body -> rewritten body ready for the
// upstream Anthropic / Codex / OpenAI call. The only side effects are the two
// module-level caches below (config mtime-cache, skill body cache).
//
// Tested directly via explicit { configPath, skillPath } opts; in production
// both paths default to on-disk locations so the HTTP handler just calls
// buildUpstreamRequest(body).

const __proxyDir = fileURLToPath(new URL(".", import.meta.url));
const DEFAULT_SKILL_PATH = join(
  __proxyDir, "..", "plugin", "skills", "gptcc-auto-delegate", "SKILL.md"
);

// mtime-based cache: re-read config only when the file changes.
let _cachedCfg = null;
...
function getConfigCached(path) {
  try {
    ...
    // Missing file / stat error: fall back to defaults via readConfig.
    _cachedCfg = readConfig(path);
    _cachedCfgPath = path;
    _cachedCfgMtime = 0;
  }
...
// Skill body cache — stable across process lifetime, keyed by path.
let _skillBody = null;
...
export function buildUpstreamRequest(body, opts = {}) {
  ...
  // Auto alias: rewrite the Claude model + prepend the delegate skill as a
  // system text block. When SuperWork is on, append the flag line so the
  // skill's own "When SuperWork is active" section triggers.
  ...
  // GPT requests: resolve Fastmode alias + strip Claude-style tone guidance
  // from system blocks.
...
// Identify ourselves honestly on every outbound request. Node's default
// fetch User-Agent ("undici") is rejected by Cloudflare bot protection on
// auth.openai.com — a real UA satisfies the WAF without impersonating
// anyone. Overridable via GPTCC_USER_AGENT.
const USER_AGENT =
  process.env.GPTCC_USER_AGENT ||
  "gptcc/2.3.1 (+https://github.com/joungminsung/gptcc)";
...
// Delegates to lib/routing.mjs with our runtime prefix list. Tests import
// the same helper module, preventing drift between tests and prod.
function isOpenAIModel(model) {
  return _isOpenAIModelBase(model, OPENAI_PREFIXES);
}
...
async function callCodexBackend(responsesReq, res, model, wantStream) {
  ...
  try {
    ...
    if (err.name === "AbortError") {
      sendError(res, 504, "api_error", "[upstream:codex] timed out after 300s");
      return;
    }
    const wrapped = new Error(`Codex fetch: ${err.message}`);
    wrapped._gptccLayer = "upstream:codex";
    throw wrapped;
  }
  ...
  if (!upstream.ok) {
    ...
    const hint =
      upstream.status === 401
        ? " (Run: gptcc login)"
        : upstream.status === 403
          ? " (Codex backend may require ChatGPT Plus/Pro, or access has been restricted)"
          : "";
    sendError(
      res,
      upstream.status,
      statusMap[upstream.status] || "api_error",
      `[upstream:codex] (${upstream.status})${hint} ${errText}`
    );
    return;
  }
...
// ============================================================================
// Section 9b: OpenAI API key fallback
// ============================================================================
//
// Alternate path when OPENAI_API_KEY is set. Uses api.openai.com/v1/responses
// directly (official API) instead of chatgpt.com/backend-api/codex (semi-public
// Codex backend). Guarantees long-term operability even if OpenAI changes the
// Codex backend. Same request/response shape, different auth.
//
// Trade-off: uses OpenAI API credits instead of the ChatGPT subscription.

const OPENAI_API_BASE = validateEndpoint(
...
async function callOpenAIAPI(responsesReq, res, model, wantStream) {
  const key = process.env.OPENAI_API_KEY;
  if (!key) {
    const err = new Error("OPENAI_API_KEY not set");
    err._gptccLayer = "config";
    err._gptccStatus = 500;
    throw err;
  }
  ...
  let upstream;
  try {
    upstream = await fetch(`${OPENAI_API_BASE}/v1/responses`, {
      method: "POST",
      headers: {
    ...
    clearTimeout(timeout);
    if (err.name === "AbortError") {
      sendError(res, 504, "api_error", "[upstream:openai] timed out after 300s");
      return;
    }
    const wrapped = new Error(`OpenAI API fetch: ${err.message}`);
    wrapped._gptccLayer = "upstream:openai";
    throw wrapped;
  }
  ...
  if (!upstream.ok) {
    ...
    sendError(
      ...
      upstream.status,
      statusMap[upstream.status] || "api_error",
      `[upstream:openai] (${upstream.status}) ${errText}`
    );
    return;
  }

  // Reuse the same SSE/non-stream translation pipeline as the Codex path —
  // the Responses API shape is identical on api.openai.com. Previously this
  // branch had two bugs:
  //   (a) translateResponseSync was called with a 3-arg shape that doesn't
  //       exist (real signature: (responsesRes, model))
  //   (b) the stream branch called translator.feed()/flush() on a translator
  //       that exposes processEvent()/end() instead.
  // Fixed to mirror callCodexBackend exactly.
  if (!wantStream) {
    clearTimeout(timeout);
    const fullText = await upstream.text();
    const outputItems = [];
    ...
    if (completedResponse) {
      ...
      sendError(
        ...
        500,
        "api_error",
        "No completed response received from OpenAI API"
      );
    }
    return;
  }
...
// ---------------------------------------------------------------------------
// Auth token (optional) — if GPTCC_AUTH_TOKEN is set, reject requests that
// don't carry the matching x-gptcc-auth or authorization header. This blocks
// other local processes from driving the proxy without authorization.
// Enforced only for /v1/messages and /model/*/invoke routes; /health stays open.
// ---------------------------------------------------------------------------

const PROXY_AUTH_TOKEN = process.env.GPTCC_AUTH_TOKEN || null;

// Delegates to the pure helper in lib/routing.mjs (same module the tests
// import). Wrapper is a thin adapter from (req) to (headers, token).
function checkProxyAuth(req) {
  return _checkProxyAuthBase(req.headers, PROXY_AUTH_TOKEN);
}

// Bedrock-format routes `/model/<id>/invoke[-with-response-stream]` are
// parsed by parseBedrockInvoke() imported from ./routing.mjs — same
// function the tests import, so they can't drift.

async function handleRequest(req, res) {
  // Health check — always open, used for proxy liveness probes
  if (req.method === "GET" && req.url === "/health") {
    ...
    res.end(
      JSON.stringify({
        ...
        features: {
          bedrockInvoke: true,
          apiKeyFallback: !!process.env.OPENAI_API_KEY,
          authRequired: !!PROXY_AUTH_TOKEN,
        },
      })
    );
    return;
  ...
  // Graceful shutdown — used by `gptcc setup` when the package was
  // upgraded and wants to respawn the proxy on the new version code.
  // Loopback-only; we already bind to 127.0.0.1 but double-check the
  // remote address so we don't accept this from an SSH tunnel.
  if (req.method === "POST" && req.url === "/shutdown") {
    const remote = req.socket?.remoteAddress || "";
    const isLoopback = remote === "127.0.0.1" || remote === "::1" || remote === "::ffff:127.0.0.1";
    if (!isLoopback) {
      sendError(res, 403, "permission_denied", "Shutdown only permitted from loopback");
      return;
    }
    res.writeHead(200, { "Content-Type": "application/json" });
  ...
  if (!checkProxyAuth(req)) {
    sendError(res, 401, "authentication_error",
      "Missing or invalid x-gptcc-auth header. Set GPTCC_AUTH_TOKEN in settings.json env to match.");
    return;
  }
  ...
  let body = {};
  try {
    if (rawBody.length > 0) body = buildUpstreamRequest(JSON.parse(rawBody.toString()));
  } catch {
    sendError(res, 400, "invalid_request_error", "Invalid JSON body");
    return;
  }

  // Bedrock-format invoke: lift model and stream flag from URL into body.
  // Claude Code (CLAUDE_CODE_USE_BEDROCK=1) sends requests to
  // /model/<id>/invoke with the body in Anthropic Messages format.
  const bedrockRoute = parseBedrockInvoke(req.url);
  if (bedrockRoute) {
    if (!body.model) body.model = bedrockRoute.model;
    if (bedrockRoute.stream && body.stream === undefined) body.stream = true;
    console.log(`[BEDROCK] ${bedrockRoute.model} stream=${bedrockRoute.stream}`);
  }
  ...
  if (isOpenAIModel(model)) {
    try {
      ...
      // API key fallback: if OPENAI_API_KEY is set, prefer the public
      // OpenAI Responses API. Codex backend is used otherwise.
      if (process.env.OPENAI_API_KEY && typeof callOpenAIAPI === "function") {
        await callOpenAIAPI(responsesReq, res, actualModel, !!body.stream);
      } else {
        await callCodexBackend(responsesReq, res, actualModel, !!body.stream);
      }
    } catch (err) {
      const layer = err._gptccLayer || "bridge";
      const status = err._gptccStatus || 500;
      sendError(res, status, "api_error", `[${layer}] ${err.message}`);
    }
  } else if (bedrockRoute) {
    // Bedrock-mode request for a Claude model: translate /model/<id>/invoke
    // back to Anthropic's /v1/messages so we can passthrough. Also add the
    // anthropic_version/bedrock_version compatibility fields if missing.
    const translatedReq = Object.create(req);
    Object.defineProperty(translatedReq, "url", {
      value: bedrockRoute.stream ? "/v1/messages" : "/v1/messages",
      configurable: true,
    });
    await passthroughToAnthropic(translatedReq, res, body, Buffer.from(JSON.stringify(body)));
  } else {
    // Body may have been rewritten by buildUpstreamRequest (Auto alias ->
    // real Opus + skill block). Forward the re-serialized body, not rawBody,
    // so those transforms reach Anthropic. Falls back to rawBody only when
    // the body was empty / unparseable (body === {}).
    const forwardBody = rawBody.length > 0 ? Buffer.from(JSON.stringify(body)) : rawBody;
    await passthroughToAnthropic(req, res, body, forwardBody);
  }
}
...
// ============================================================================
//
// Gate all startup side effects (auth pre-flight, port bind, signal handlers)
// behind a "direct run" check. This lets unit tests `import` from proxy.mjs
// (for buildUpstreamRequest, etc.) without loading Codex auth or binding a
// TCP port. bin/gptcc.mjs runs the proxy via `await import("../lib/proxy.mjs")`
// which resolves to the same file URL as process.argv[1] once the harness
// invokes `node .../proxy.mjs`, so production startup is preserved.

function isDirectRun() {
  // True when the process was started to actually run the proxy — either
  // `node .../proxy.mjs` (production spawn from setup.mjs / start-proxy.mjs)
  // or `gptcc proxy` (bin/gptcc.mjs dispatches via dynamic import). False
  // when another module imports proxy.mjs for its exports (unit tests).
  try {
    const argv1 = process.argv[1] || "";
    if (argv1.endsWith("proxy.mjs")) return true;
    // bin/gptcc.mjs `proxy` subcommand.
    if (argv1.endsWith("gptcc.mjs") && process.argv[2] === "proxy") return true;
    // Installed bin shim (`gptcc`) -> argv[1] has no extension.
    if (/[\\/]gptcc$/.test(argv1) && process.argv[2] === "proxy") return true;
    return false;
  } catch {
    return false;
  }
...
if (isDirectRun()) {
  ...
  server.listen(PORT, "127.0.0.1", () => {
    console.log(`GPT for Claude Code Proxy v2.3.1 on http://127.0.0.1:${PORT}`);

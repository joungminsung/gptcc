...
const USER_AGENT =
  ...
  "gptcc/2.2.0 (+https://github.com/joungminsung/gptcc)";
...
async function callOpenAIAPI(responsesReq, res, model, wantStream) {
  ...
  // Reuse the same SSE/non-stream translation pipeline as Codex — the
  // Responses API shape is identical on api.openai.com.
  if (!wantStream) {
    clearTimeout(timeout);
    const fullText = await upstream.text();
    const outputItems = [];
    ...
    const translated = translateResponseSync(completedResponse, outputItems, model);
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify(translated));
    return;
  }
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(`GPT for Claude Code Proxy v2.2.0 on http://127.0.0.1:${PORT}`);

...
export async function doctor() {
  ...
  if (existsSync(claudeBin)) {
    ...
    failed++;
    line(FAIL, `Claude Code binary not found`,
      `Searched: ${claudeBin}`,
  ...
  if (!existsSync(SETTINGS_PATH)) {
    ...
    try {
      ...
      if (authToken && authToken.startsWith("gptcc_")) {
        line(OK, "Proxy auth token set");
      } else if (authToken) {
        line(OK, "Proxy auth token set (non-gptcc)");
      } else {
        warned++;
        line(WARN, "No proxy auth token — local processes can drive the proxy",
          null,
          "Run: gptcc setup  (generates a fresh token)");
      }
    } catch (err) {
      failed++;

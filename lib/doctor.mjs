...
export async function doctor() {
  ...
  if (!existsSync(SETTINGS_PATH)) {
    ...
    try {
      ...
      // MCP server registration — the single biggest lever for Claude
      // actually *calling* GPT proactively. Without it, ask_gpt54 /
      // review_with_gpt54 don't appear in Claude's tool list, so
      // delegation stays manual.
      if (s.mcpServers?.gptcc?.command) {
        line(OK, "MCP server registered (ask_gpt54 / review_with_gpt54 tools)");
      } else {
        warned++;
        line(WARN, "MCP server not registered — Claude won't auto-call GPT",
          null,
          "Run: gptcc setup  (adds mcpServers.gptcc entry)");
      }
    } catch (err) {
      failed++;
  ...
  try {
    ...
    } else if (/unknown command|not a git command/i.test(out)) {
      // Claude Code CLI build without plugin subcommand support. The
      // plugin is only needed for SessionStart auto-restart; the proxy
      // is already running, so this is not user-actionable.
      line(OK, "plugin step not supported by this Claude Code build (non-blocking)");
    } else {
      line(OK, "plugin not registered (optional — used only for SessionStart auto-restart)");
    }
  } catch (err) {
    // `plugin list` returning non-zero doesn't block anything gptcc does
    // — proxy already runs standalone. Report it as informational, not
    // as a user-facing warning.
    const msg = (err?.stderr?.toString() || err?.message || "").split("\n")[0];
    if (/unknown command|not a git command/i.test(msg)) {
      line(OK, "plugin step not supported by this Claude Code build (non-blocking)");
    } else {
      line(OK, "plugin list unavailable (non-blocking)");
    }
  }
  }  // end if (claudeBin)
